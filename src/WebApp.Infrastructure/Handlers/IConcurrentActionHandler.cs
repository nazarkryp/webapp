...
namespace WebApp.Infrastructure.Handlers
{
    public interface IConcurrentActionHandler
    {
        ...
        Task ForAsync<TResult>(Func<int, Task<TResult>> action, int fromInclusive, int toExclusive, int maxDegreeOfParallelism, Func<IEnumerable<TResult>, Task> iterationCompleted, CancellationToken? token = null);
